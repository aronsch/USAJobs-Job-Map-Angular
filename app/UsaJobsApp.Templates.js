...
(function () {
	...
	function configTemplates ($templateCache) {
		...
		// Note: Leaflet.js map template/HTML generation is located in Maps module jobMapDirective
		...
		function jobTableTemplate () {
			...
			// Title
			tmplt += '<dl class="col-xs-6 col-sm-1 col-md-1 col-lg-1 small"><dt><a class="usajobs-sortbutton"' + activeClass('JobTitle') + ' title="Sort by Job Title" href="" ng-class="" ng-click="setPredicate(\'JobTitle\')">Title'
					+ sortCaret('JobTitle', alphaIcons) + '</a></dt></dl>';
			// Locations
			tmplt += '<dl class="hidden-xs col-sm-2 col-md-1 col-lg-1 small"><dt><a class="usajobs-sortbutton"' + activeClass('Location') + ' title="Sort by Location" href="" ng-click="setPredicate(\'Locations\')">Location'
					+ sortCaret('Locations', alphaIcons) + '</a></dt></dl>';
			// Salary
			tmplt += '<dl class="col-xs-6 col-sm-3 col-md-2 col-lg-2 small"><dt><a class="usajobs-sortbutton"' + activeSalaryClass() + ' title="Sort by Salary" href="" ng-click="setSalaryPredicate()">Salary'
					+ sortSalaryCaret('salaryPredicate') + '</a></dt></dl>';
			// Grade
			tmplt += '<dl class="col-xs-6 col-sm-2 col-md-2 col-lg-2 small"><dt><a class="usajobs-sortbutton"' + activeClass('Grade') + ' title="Sort by Pay Grade" href="" ng-click="setPredicate(\'Grade\')">Grade'
					+ sortCaret('Grade', amntIcons) + '</a></dt></dl>';
			// Job Close Date
			tmplt += '<dl class="col-xs-6 col-sm-2 col-md-2 col-lg-2 small"><dt><a class="usajobs-sortbutton"' + activeClass('daysRemaining') + ' title="Sort by Announcement Closing Date" href="" ng-click="setPredicate(\'daysRemaining\')">Closes'
					+ sortCaret('daysRemaining', defIcons) + '</a></dt></dl>';
			// Job Open Date
			tmplt += '<dl class="col-xs-6 col-sm-2 col-md-2 col-lg-2 small"><dt><a class="usajobs-sortbutton"' + activeClass('daysOpen') + ' title="Sort by Announcement Opening Date" href="" ng-click="setPredicate(\'daysOpen\')">Opened'
					+ sortCaret('daysOpen', defIcons) + '</a></dt></dl>';
			// Job Announcement Number
			tmplt += '<dl class="hidden-xs hidden-sm hidden-md col-lg-2 small"><dt><a class="usajobs-sortbutton"' + activeClass('AnnouncementNumber') + ' title="Sort by Job Announcement Number" href="" ng-click="setPredicate(\'AnnouncementNumber\')"><abbr title="Vacancy Announcment Number">Vac #</abbr>'
			...
			tmplt += '<i class="fa fa-lg fa-times-circle"></i> No jobs match your current filter criteria <span class="btn btn-danger"><i class="fa fa-close"></i> Clear filters</span>';
			tmplt += '</li>'
			...
			tmplt += '<h5 class="usajobs-jobs-table-list-item-title"><a ng-href="{{job.ApplyOnlineURL}}" target="_blank" title="Open this job announcement on USAJobs.gov">{{ job.title }}<i class="fa fa-fw fa-external-link"></i>';
			tmplt += '</a></h5>';
			tmplt += '<div class="row usajobs-table-item-job-details">';
			// Locations
			tmplt += '<dl class="col-xs-12 col-sm-3 col-md-2 col-lg-2 small"><dt>Location</dt><dd ng-repeat="loc in job.locationArrayCompact track by $index" title="{{ loc }}">{{ loc }}</dd></dl>';
			// Salary                   
			tmplt += '<dl class="col-xs-12 col-sm-3 col-md-2 col-lg-2 small"><dt>Salary</dt><dd>{{ job.salaryRange }}<span ng-show="job.hourly()">/hour</span></dd></dl>';
			// Grade                    
			tmplt += '<dl class="col-xs-12 col-sm-2 col-md-2 col-lg-2 small"><dt>Grade</dt><dd>{{ job.gradeRangeDesc() }}</dd></dl>';
			// Job Close Date           
			tmplt += '<dl class="col-xs-12 col-sm-2 col-md-2 col-lg-2 small"><dt>Closes</dt><dd title="{{ job.EndDate }}">{{ job.endDateDescription }}</dd></dl>';
			// Job Open Date            
			tmplt += '<dl class="col-xs-12 col-sm-2 col-md-2 col-lg-2 small"><dt>Opened</dt><dd>{{ job.StartDate }}</dd></dl>';
			// Job Announcement Number  
			tmplt += '<dl class="hidden-xs hidden-sm hidden-md col-lg-2 small"><dt><abbr title="Vacancy Announcment Number">Vac</abbr></dt><dd title="{{job.AnnouncementNumber}}">{{ job.AnnouncementNumber }}</dd></dl>';
			// Job Summary Toggle
			tmplt += '<div class="col-xs-12 col-sm-12 col-md-2 col-lg-2 clearfix"><a class="hidden-print small text-muted" ng-click="job.toggleDescription()"><i class="fa fa-fw" ng-class="{ \'fa-plus-square-o\': !job.showDescription, \'fa-minus-square-o\': job.showDescription }"></i>Job Summary</a></div>'
			tmplt += '<dl class="col-xs-12 col-sm-12 col-md-10 col-lg-10 small" ng-show="job.showDescription"><dt class="sr-only visible-print-block">Job Summary</dt><dd><strong class="visible-xs-inline visible-sm-inline visible-md-inline">Vacancy Number: {{job.AnnouncementNumber}}<br /></strong> {{ job.JobSummary }}</dd></dl>';
			tmplt += '</div>';
			tmplt += '</li></ul>';
			tmplt += '</div></div>';
